...
public class RBTree {
	...
	private void rb_Insert_Fixup(Node node) {
		...
		while (node.getParent().getColor() == Color.RED) {
			...
			if (node.getParent() == node.getParent().getParent().getLeft()) { //父节点是祖父节点左孩子
				
				Node rightNuncle = node.getParent().getParent().getRight();
				
				if (rightNuncle.getColor() == Color.RED) {         //Case 1叔节点为红
				...
				} else if (node == node.getParent().getRight()) {  //case 2 子节点为父节点右孩子
